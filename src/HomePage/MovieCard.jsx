...
const MovieCard = ({movieId, setCurrentMovie, poster, rating, title, date, lang, genreIds, genres}) => {
    ...
    return (
        <Link to={"/player"}>
            <div onClick={() => {setCurrentMovie(movieId);}} className={"z-0 w-full p-[8px] rounded-lg bg-gradient-to-br from-purple-950 to-[rgba(0,0,255,.13)] max-md:h-[290px] max-md:w-[150px] max-md:min-w-[140px] flex flex-col items-start justify-between max-md:p-[4px]"}>
                <img className={"max-h-full rounded-md"} src={poster?`https://image.tmdb.org/t/p/w500/${poster}`:'/project-images/No-Poster.png' } alt="poster"/>
