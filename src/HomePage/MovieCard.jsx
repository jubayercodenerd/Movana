            ...
            <div onClick={() => {setCurrentMovie(movieId);}} className={"z-0 h-[340px] w-[180px] p-[8px] rounded-lg bg-gradient-to-br from-purple-950 to-[rgba(0,0,255,.13)] max-md:h-[295px] max-md:w-[160px] max-md:min-w-[160px] flex flex-col items-start justify-between"}>
                ...
                <div className={"flex flex-col items-start justify-between"}>
                    ...
                    <div className={"flex flex-wrap justify-start items-center mt-[3px] gap-[2px]"}>
                        {
                            genreNames[0]?<p className={"text-[10px] max-md:text-[10px] font-semibold text-gray-300"}>{genreNames?.[0]?.name || "N/A"}</p>:<></>
                        }
                        {
                            genreNames[1]?<><p className={"text-[10px] max-md:text-[10px] font-semibold text-gray-300"}>•</p><p className={"text-[10px] max-md:text-[10px] font-semibold text-gray-300"}>{genreNames?.[1]?.name || "N/A"}</p></>:<></>
                        }
                        {
                            genreNames[2]?<><p className={"text-[10px] max-md:text-[10px] font-semibold text-gray-300"}>•</p><p className={"text-[10px] max-md:text-[10px] font-semibold text-gray-300"}>{genreNames?.[2]?.name || "N/A"}</p></>:<></>
                        }
                    </div>
                </div>
            </div>
